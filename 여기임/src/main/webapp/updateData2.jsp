...
try {
    ...
    JSONArray rows = new JSONArray(jsonData);
        ...
        for (int i = 0; i < rows.length(); i++) {
            ...
            // 업데이트 쿼리
            String updateQuery = " insert into testTable(serialNumber, medicineName, buyingPrice, price, inventory, companyName, standard, receiptDate, deliveryDate, returnInv, kind) "
            				   + " values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
