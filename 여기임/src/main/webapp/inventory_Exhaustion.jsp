...
.dataDetail .content, .dataDetail .text {
	display: none; /* 처음에는 숨김 */
	padding: 20px; /* 패널 확장 후 내용 보기 쉽게 */
}

.dataDetail.expansion {
	width: 40%; /* 마우스 올리면 40%로 확장 */
}
...
<body style="overflow: hidden">
	<div id="wrap" style="overflow-y: hidden; text-align: center;">
		...
		<div id="data" class="data"
			...
			<div class="rightUI" style="margin-top: -30%;">
				...
				<table class="table_line"
					style="width: 100%; border-collapse: collapse;margin-top:20%;">
					<colgroup>
						<col width="7%">
						<col width="13%">
						<col width="6%">
				...
				<!-- 두 번째 테이블 (상품 리스트) -->
				<div id="scrollTable"
					style="max-width: calc(100%); max-height: calc(55vh); overflow-y: auto; width: 100%;">
						...
						<tbody id="table_body">
							<tr>
								...
								<td><input type="text" name="standard" id="standard"
									class="standard"
									style="width: 100%; border: revert-layer; outline: none; background: none; text-align: center;"></td>
								<td><input type="text" name="Buyingprice" id="Buyingprice"
			...
			<div class="leftUI"
				...
				<div class="herbalSet">
					<div class="button">
						<p class="btnText">일반세트</p>
						<div class="btnTwo">
							<p class="btnText2">만들기</p>
						</div>
					</div>
					<table class="regularTable" border="1"
						...
						<tbody>
							<tr>
								...
								try {
									...
									// 데이터 조회
									String sql = "SELECT medicineName  FROM productSet ";
							...
							<tr>
								...
								%>
								<td style="height: 50px;"><%=medicineName%></td>
								<%
								count++;
								}
								if (count % 4 != 0) {
						...
						<tbody>
							<tr>
								...
								try {
									...
									// 데이터 조회
									String sql = "SELECT medicineName FROM regularSet";
		...
		<div class="dataDetail" style="z-index: 1000;">
			<div class="icons">
				<i class="icon1">🔍</i> <i class="icon2">➕</i>
			</div>
			<div class="content">
				<h2>판매 상세정보</h2>
				<input type="text" class="medicineNameSearch">
				<table border=1
					style="border-collapse: collapse; width: 100%; height: 20vh; display: block; overflow-y: scroll;">
					<thead>
						<tr>
							<th>날짜</th>
							<th>구매자 성함</th>
	...
	$(document).ready(function() {
		...
		$(document).mouseover(function() {
		    if (
		        !$("#table_body .inventory, .table_line .clientName, .table_line .clientNumber, .standard, .price, #medicinePrice, #generalPrice").is(":focus") &&
		        !$(".dataDetail").find("input, textarea").is(":focus")  // 패널 안의 input이 포커스 되어 있으면 예외 처리
		    ) {
		        $('#SerialNumber').focus();
		    }
		});
		...
		$(".dataDetail .icon1").on("click", function() {
		    isInside = true;
		    $(".dataDetail").stop(true, true).animate({ width: "40%" }, 300, function() {
		        $(".dataDetail").find(".icons").fadeOut(100); // 아이콘 숨기기
		        $(".dataDetail").find(".content").fadeIn(300);
		    });
		});

		$(".dataDetail .icon2").on("click", function() {
		    isInside = true;
		    $(".dataDetail").stop(true, true).animate({ width: "40%" }, 300, function() {
		        $(".dataDetail").find(".icons").fadeOut(100); // 아이콘 숨기기
		        $(".dataDetail").find(".text").fadeIn(300);
		        $(this).css("opacity", "1"); // 현재 요소의 불투명도 조정
		    });
		});

		// mouseleave 이벤트 통합
		$(".dataDetail").on("mouseleave", function() {
		    isInside = false; // isInside만 사용
		    setTimeout(() => {
		        if (!isInside && !$(this).find("input, textarea").is(":focus")) {
		            // content와 Income을 동시에 숨기고 아이콘을 보이도록 설정
		        	$(this).find(".content, .text").fadeOut(200, function() {
		                $(this).parent().find(".icons").fadeIn(200); // 아이콘 보이기
		                $(this).parent().stop(true, true).animate({ width: "5%" }, 300);
		            });
		        	$(this).find(".content, .content").fadeOut(200, function() {
		                $(this).parent().find(".icons").fadeIn(200); // 아이콘 보이기
		                $(this).parent().stop(true, true).animate({ width: "5%" }, 300);
		            });
		        }
		    }, 100); // 약간의 딜레이를 줘서 포커스를 체크
		});



		//사용자 성함 검색하려고 할 때, focus가 SerialNumber로 안넘어가지도록 고정
	    $(".dataDetail input, .dataDetail textarea").on("focus", function(){
	        isInside = true;  // 입력 필드가 포커스를 가지면 닫히지 않게
	    }).on("blur", function(){
	        setTimeout(() => {
	            if (!isInside) {
	                $(".dataDetail .content").fadeOut(200, function(){
	                    $(".dataDetail").stop(true, true).animate({ width: "5%" }, 300);
	                });
	            }
	        }, 200);  // blur가 된 후에 닫히는 것을 체크
	    });
	    
		//고객이 어떤 제품을 구매했는지 보여주도록 하는 ajax
	    $(".dataDetail .content .medicineNameSearch").on("keyup", function(e){
	        var medicineVal = $(this).val();  // input의 값을 가져옴
	        if(e.keyCode == 13){
	            var that = $(this);  // 현재 요소를 저장
	            $.ajax({
	                url : "clientVal.jsp",
	                method: "POST",
	                data : {medicineVal: medicineVal},
	                success : function(response){
	                    console.log("성공");
	                    $(".realData").html(response);  // response로 HTML을 업데이트
	                },
		...
		//제품이 언제 몇개가 들어왔는지 보여주는 ajax
 		$(".dataDetail .text .medicineNameSearch").on("keyup", function(e){
	        var medicineVal = $(this).val();  // input의 값을 가져옴
	        if(e.keyCode == 13){
	            var that = $(this);  // 현재 요소를 저장
	            $.ajax({
	                ...
	                success : function(response){
	                    console.log("성공");
	                    $(".realData2").html(response);  // response로 HTML을 업데이트
	                },
	                error: function(response){
	                    console.log("실패");
	                }
 		...
 		$(".herbalSet .button").on("click", function(){
 			location.href='setMake.jsp';  
 		});
 		$(".regularSet .button").on("click", function(){
 			location.href='setMake2.jsp';  
 		});
		
		//한약세트 클릭 이벤트들
$(".leftUI .regularSet .regularTable tr td").on("click", function() {
    var clickName = $(this).text().trim(); // 클릭한 td의 텍스트
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            try {
                ...
                var medicineName = jsonData.medicineName.trim();
	...
	$("#exhaustionInv").on("click", function() {
	    ...
	    rowData.push({ generalPrice: generalPrice, medicinePrice: medicinePrice, discounted:discounted });
	    console.log(rowData);
	    // AJAX 요청
	    $.ajax({
