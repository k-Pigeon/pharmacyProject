...
try {
    ...
    String updateSql =
        ...
        " WHERE DeliveryDate = ? AND medicineName = ? AND SerialNumber = ? AND domain_type = ?";
    ...
    while (rs.next()) {
        ...
        psUpdate.setString(5, domainType);
        updated += psUpdate.executeUpdate();
    }

    // 3) 삭제
    String delSales = "DELETE FROM SalesRecord  WHERE saleDate = ? AND domain_type = ?";
    psDelSales = conn.prepareStatement(delSales);
    psDelSales.setString(1, saleDate);
    psDelSales.setString(2, domainType);
    deletedSales = psDelSales.executeUpdate();

    String delClient = "DELETE FROM clientRecord WHERE saleDate = ? AND domain_type = ?";
    psDelClient = conn.prepareStatement(delClient);
    psDelClient.setString(1, saleDate);
    psDelClient.setString(2, domainType);
