...
try {
    ...
    for (int i = 0; i < jsonArray.length() - 1; i++) {
        ...
        String sortationSQL = "UPDATE testTable "
        		   + "SET price = ? "
        		   + "WHERE serialNumber = ? AND medicineName = ? AND standard = ? AND domain_type = ? ";
        ...
        ptement.setString(5, domainType);
        ...
        String recordSQL = "INSERT INTO fluctuationRecord (saleDate, medicineName, price, domain_type) "
        				 + "VALUES(?, ?, ?, ?)";
        recordPtement = conn.prepareStatement(recordSQL);
        ...
        recordPtement.setString(4, domainType);
    ...
    try { if (recordPtement != null) recordPtement.close(); } catch (Exception e) {}
