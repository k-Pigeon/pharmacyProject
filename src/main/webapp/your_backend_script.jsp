...
try {
    ...
    // 개별 파라미터 삽입
    String priceSql = "INSERT INTO priceRecord (saleDate, maxmedicinePrice, generalPrice, domain_type) VALUES (?, ?, ?, ?)";
    ...
    for (int i = 0; i < jsonArray.length() - 1; i++) {
        ...
        String sortationSQL = "SELECT kind FROM testTable WHERE SerialNumber = ? AND medicineName = ? AND domain_type = ? ";
        ...
        ptement.setString(3, domainType);
        ...
        String standardSortationSQL = "SELECT standard FROM testTable WHERE standard LIKE '%10병%' "
                                    + "AND medicineName = ? AND domain_type = ? "
        ...
        pstement.setString(2, domainType);
        pstement.setString(3, obj.getString("serialNumber"));
        pstement.setString(4, obj.getString("medicineName"));
        ...
        if ("드링크류".equals(kind) && "10병".equals(standard)) {
            sql = "UPDATE testTable SET inventory = inventory - ? "
                 + "WHERE medicineName = ? AND domain_type = ? "
                 + "AND DeliveryDate = (SELECT minDate FROM (SELECT MIN(DeliveryDate) AS minDate "
                 + "FROM testTable WHERE SerialNumber = ? AND medicineName = ? AND returnInv = '0') AS subquery)";
            statement1 = conn.prepareStatement(sql);
            // 파라미터 설정
            statement1.setInt(1, obj.getInt("inventory") * 10);
            statement1.setString(2, domainType);
            statement1.setString(3, obj.getString("medicineName"));
            statement1.setString(4, obj.getString("serialNumber"));
            statement1.setString(5, obj.getString("medicineName"));
            statement1.executeUpdate();
        } else {
            // SQL 쿼리 작성
            sql = "UPDATE testTable SET inventory = inventory - ? "
                 + "WHERE SerialNumber = ? AND medicineName = ? AND domain_type = ? "
                 + "AND DeliveryDate = (SELECT minDate FROM (SELECT MIN(DeliveryDate) AS minDate "
                 + "FROM testTable WHERE SerialNumber = ? AND medicineName = ? AND returnInv = '0') AS subquery)";
            ...
            statement.setString(4, domainType);
            statement.setString(5, obj.getString("serialNumber"));
            statement.setString(6, obj.getString("medicineName"));
            statement.executeUpdate();
        }

        // 판매 날짜 입력
        String salesSql = "INSERT INTO SalesRecord (saleDate, medicineName, Buyingprice, price, inventory, domain_type) VALUES (?, ?, ?, ?, ?, ?)";
