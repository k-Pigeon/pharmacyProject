<%@ page language="java" contentType="application/json; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ page import="java.sql.*"%>
<%@ include file="DBconnection.jsp"%>
<%
String dbName = (session != null) ? (String) session.getAttribute("dbName") : null;
String id = (session != null) ? (String) session.getAttribute("id") : null;
String password = (session != null) ? (String) session.getAttribute("password") : null;

String domainType = (session != null) ? (String) session.getAttribute("domainType") : null; if (id == null || dbName == null) {
	response.sendRedirect("login.jsp");
	return;
}

response.setContentType("application/json");
response.setCharacterEncoding("UTF-8");

PreparedStatement pstmt = null;
ResultSet rs = null;

try {
	String serialNumber = request.getParameter("serialNumber");
	if (serialNumber == null || serialNumber.isEmpty()) {
		out.println("{\"error\":\"serialNumber is missing\"}");
		return;
	}

	String sql = " select medicineName, Buyingprice, price, inventory, kind, standard, companyName, DeliveryDate, quantity "
	+ " from testTable where SerialNumber = ? AND domain_type = ?";
	pstmt = conn.prepareStatement(sql);
	pstmt.setString(1, serialNumber);
	pstmt.setString(2, domainType);
	rs = pstmt.executeQuery();

	if (rs.next()) {

		String medicineNames = rs.getString("medicineName");
		String deliveryDate = rs.getString("DeliveryDate");
		String buyingPrice = rs.getString("Buyingprice");
		String price = rs.getString("price");
		String kind = rs.getString("kind");
		String companyName = rs.getString("companyName");
		String standard = rs.getString("standard");
		String quantity = rs.getString("quantity");

		double inventory = rs.getDouble("inventory"); // 원래 inventory 값

		String jsonData = "{" + "\"medicineName\":\"" + rs.getString("medicineName")
		+ "\"," + "\"Buyingprice\":\""	+ rs.getString("Buyingprice")
		+ "\"," + "\"price\":\"" + rs.getString("price") + "\","
		+ "\"inventory\":\"" + inventory + "\","
		+ "\"standard\":\"" + rs.getString("standard")
		+ "\"," + "\"kind\":\"" + rs.getString("kind")
		+ "\"," + "\"companyName\":\"" + rs.getString("companyName")
		+ "\"," + "\"quantity\":\"" + rs.getString("quantity") + "\","
		+ "\"DeliveryDate\":\"" + rs.getString("DeliveryDate") + "\""
		+ "}";
		out.print(jsonData);
	} else {
		out.println("{\"error\":\"No data found for SerialNumber: " + serialNumber + "\"}");
	}
} catch (SQLException e) {
	e.printStackTrace();
	out.println("{\"error\":\"Database error: " + e.getMessage() + "\"}");
} finally {
    try { if (pstmt != null) pstmt.close(); } catch (Exception ignored) {}
    try { if (rs != null) rs.close(); } catch (Exception ignored) {}
    if (conn != null) { try { conn.close(); } catch (Exception ignore) {}
  }
}
%>
