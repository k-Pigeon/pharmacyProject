...
try {
    ...
    for (String key : data.keySet()) {
        ...
        /* -------------------------------
           1. inventory 계산 (표시 수치 → 실제 수치)
        -------------------------------- */
        if (row.has("inventory")) {
            row.put("inventory", cleanNumber(row.getString("inventory")));
        }
        ...
        /* -------------------------------
           4. UPDATE SQL 생성 (안전한 방식)
        -------------------------------- */
        StringBuilder sql = new StringBuilder(
            "UPDATE testTable" + idSortation + " SET "
        );

        List<String> colNames = new ArrayList<>();
        for (String col : row.keySet()) colNames.add(col + "=?");
        ...
        for (String col : row.keySet()) {
            pstmt.setString(idx++, row.getString(col));
        }
