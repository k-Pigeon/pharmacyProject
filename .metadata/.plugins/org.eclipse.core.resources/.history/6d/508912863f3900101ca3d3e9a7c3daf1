...
<body>
				...
				<table class="table_line"
					...
					<tbody id="inventory_list">
						...
						try {
							...
							String sql = "SELECT A.saleDate AS Dates FROM SalesRecord A " + "JOIN priceRecord B ON A.saleDate = B.saleDate";
	...
	<script>
        ...
        $(".updateBtn").on("click", function(event) {
            ...
            var timeToset = row.find("td:eq(2)").text();  // saleDate 가져오기
        ...
        $(".deleteBtn").on("click", function(event){
            ...
            var buttonText = row.find("td:eq(3) .updateBtn").text();
