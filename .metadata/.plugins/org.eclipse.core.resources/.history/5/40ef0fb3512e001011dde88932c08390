    ...
    for (int i = 0; i < jsonArray.length() - 2; i++) {
        ...
        if ("일반세트".equals(obj.optString("serialNumber", ""))) {
            ...
            while (inventoryAdjustment > 0) {
                ...
                double standardFactor = 1.0; // 기본값은 1

                // 1️⃣ **띄어쓰기가 있는 경우 (두 개의 숫자를 곱함 → 최우선 조건!)**
                ...
                // 2️⃣ **ml이 포함된 경우 → `inventory * standard` 적용**
                ...
                // 3️⃣ **g, l, 포가 포함된 경우 → inventory 그대로 유지 (standardFactor = 1)**
                else if (standard.matches(".*(g|l|포)$")) {
                    standardFactor = 1;
                }
                // 4️⃣ **모든 조건이 안 맞으면 → `inventory * standard 숫자` 적용**
                else {
                    try {
                        standardFactor = Double.parseDouble(standard.replaceAll("[^0-9]", ""));
                    } catch (Exception e) {
                ...
                PreparedStatement updateStmt = conn.prepareStatement(updateSQL);
                updateStmt.setDouble(1, deductedInventory);
                updateStmt.setDouble(2, calculatedInventory); // ml일 경우 1.0, 아닌 경우 계산된 standardFactor 사용
