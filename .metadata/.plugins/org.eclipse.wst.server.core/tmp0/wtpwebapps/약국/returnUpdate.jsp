...
<%
    ...
    try {
        ...
        if("반품됨".equals(returnText)) {
            sql = "UPDATE testTable"  + idSortation + "  SET returnInv = '0' WHERE medicineName = ? AND DeliveryDate = ? AND standard = ? ";
        } else if("반품하기".equals(returnText)) {
            sql = "UPDATE testTable"  + idSortation + "  SET returnInv = '1' WHERE medicineName = ? AND DeliveryDate = ? AND standard = ? ";
        }
