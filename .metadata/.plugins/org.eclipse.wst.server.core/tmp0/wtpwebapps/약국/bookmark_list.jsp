			...
			<table id="table_line">
				...
				<tbody id="inventory_list">
					...
					try {
						...
						conn = DriverManager.getConnection(jdbcDriver, id, password);

						if (request.getMethod().equalsIgnoreCase("post")) {
							SQLCondition = request.getParameter("barcodeInput");
							//값을 검색했을 때 사용하는 SQL
							sql = "SELECT " + " MAX(medicineName) AS medicineName, " + " MAX(price) AS price, "
							+ " CAST(SUM(inventory) AS UNSIGNED) AS totalInventory, " + " MAX(kind) AS kind, "
							+ " MAX(PlaceClassification) AS PlaceClassification, " + " MAX(companyName) AS companyName, "
							+ " MAX(standard) AS standard, " + " MAX(receiptDate) AS receiptDate, DeliveryDate " + " FROM testTable"
							+ idSortation + "  WHERE Bookmark = '1' " + " AND PlaceClassification IN ('약국', '지하') "
							+ " AND SerialNumber = ? " + " GROUP BY medicineName, DeliveryDate " + " ORDER BY DeliveryDate ASC";
							...
							+ " MAX(standard) AS standard, " + " MAX(receiptDate) AS receiptDate, DeliveryDate " + " FROM testTable"
							+ idSortation + "  WHERE Bookmark = '1' " + " AND PlaceClassification IN ('약국', '지하') "
							+ " GROUP BY medicineName, DeliveryDate " + "ORDER BY DeliveryDate ASC";
							pstmt = conn.prepareStatement(sql);
						}
