...
<%
    ...
    try {
        ...
        // 쿼리 작성
        String sql = "SELECT Bookmark FROM testTable"  + idSortation + "  "
        ...
        if(rs.next()) {
            ...
            if("0".equals(bookmark)){ // 문자열 비교 수정
                String chSQL = " UPDATE testTable"  + idSortation + "  SET Bookmark = '1' "
            ...
            if("1".equals(bookmark)){ // 문자열 비교 수정
                String chSQL = " UPDATE testTable"  + idSortation + "  SET Bookmark = '0' "
                             + " WHERE medicineName = ? AND DeliveryDate = ?";
        ...
        if(ch2Pstmt != null) {
            try { ch2Pstmt.close(); } catch(SQLException e) { e.printStackTrace(); }
        }
        if(rs != null) {
            try { pstmt.close(); } catch(SQLException e) { e.printStackTrace(); }
        }
        if(chRS != null) { // 변경된 PreparedStatement를 추가로 닫아야 함
